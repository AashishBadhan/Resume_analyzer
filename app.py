...
st.markdown("""
    ...
    .sub-header {text-align: center; color: #888; font-size: 18px; margin-bottom: 40px;}
    ...
    .stButton>button {
        ...
        color: white; border: none; padding: 0.6rem; transition: all 0.3s ease;
    }
    .stButton>button:hover {transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.2);}
    
    /* Metrics Cards */
    [data-testid="stMetricValue"] {font-size: 28px !important; font-weight: 800 !important; color: #1e3c72;}
    [data-testid="stMetricLabel"] {font-size: 14px !important; font-weight: 600 !important; color: #555;}
    ...
    /* Expanders */
    .streamlit-expanderHeader {font-size: 16px; font-weight: 700; color: #1e3c72; background-color: #f1f5f9; border-radius: 8px;}
    </style>
...
def clean_text(text):
    ...
    text = re.sub(r'[\r\n\t]', ' ', text) # Newlines
    text = re.sub(r'[^\w\s\.\@\-\+]', ' ', text) # Keep email/phone characters
...
def extract_phone(text):
    ...
    for match in matches:
        clean_match = re.sub(r'[\s\-]', '', match) # Remove spaces and dashes
...
def extract_experience(text):
    # Catches: "2 years", "2.5 yrs", "10+ years", "5 months"
